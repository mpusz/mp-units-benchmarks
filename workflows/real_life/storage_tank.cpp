...
// Frozen snapshot of mp-units' example/storage_tank.cpp (as of a742af8c4), lightly reshaped to the
// corpus preamble convention. The real_life/ tier prices the MIX the sharp workflows deliberately
// avoid - custom quantity specs with a constrained equation, a class hierarchy over typed
// quantities, chrono interop, iostream plus std::format output, all in one TU. It is still far
// SMALLER than a production file: size is the scaling/ series' axis, and a production TU's cost is
// roughly its include intercept plus the slopes times its size - this tier only keeps the mix
// honest. The snapshot is deliberately FROZEN here rather than included from the measured
// checkout: a workflow whose source changed with the ref would compare different programs and call
// it a regression.
