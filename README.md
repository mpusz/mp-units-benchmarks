...
| `scaling/`   | the **slope**: the same work at 16, 64 and 256 steps, so cost-per-operation falls out of the difference. `narrow_*` reuses five quantity types (a production file's shape); `broad_*` composes a distinct derived unit per step; `typed_broad_*` mirrors `broad` with typed references, so their slope difference is the safety-level-5 tax per derived quantity (measured 2.46×, confirmed by the safety ladder's 2.47×); `specs_broad_*` composes bare spec expressions (constraint algebra alone); and the `define_*` shapes define one entity per step — a named unit, a leaf quantity spec, a measured constant — so the cost of a *definition* is a slope too, immune to library growth by construction |
...
| `real_life/` | frozen snapshots of mp-units' own `example/` programs — the **mix** the sharp workflows avoid: `storage_tank` (custom specs with equations, a class hierarchy over typed quantities, chrono interop, formatted output) and `glide_computer` (~870 lines: kind-safe geographic coordinates with bounded origins and frame projections, custom formatters, ranges algorithms over typed quantities, a simulation loop). Still smaller than production code: size is `scaling/`'s axis, this tier keeps the mix honest |
